...
function build3DModelWithPatternLoading(
  ...
  const nodeSprings = new Map<string, [number, number, number, number, number, number]>();
  ...
  const getOrCreateNode = (
    ...
    // 1. Resolve manual restraints or springs
    const xM = x / 1000;
    const yM = y / 1000;
    const searchKey = `${xM.toFixed(2)}_${yM.toFixed(2)}_${z}`;
    const customRestraint = supportRestraints?.[searchKey];

    const colNear = columns.find(c => !c.isRemoved && Math.abs(c.x * 1000 - x) < 5 && Math.abs((c.zBottom ?? 0) - z) < 5);

    let springRestraints: [boolean, boolean, boolean, boolean, boolean, boolean] | undefined;
    let computedSprings: [number, number, number, number, number, number] | undefined;

    if (colNear && foundationDb) {
      const assignment = (foundationDb.assignments || []).find((a: any) => a.supportedId === colNear.id && a.supportedType === "column");
      if (assignment) {
        const geom = (foundationDb.geometries || []).find((g: any) => g.foundationId === assignment.foundationId);
        const soil = (foundationDb.soils || []).find((s: any) => s.foundationId === assignment.foundationId);
        if (geom && soil) {
          const ks = soil.modulusSubgrade || (soil.qall * 120);
          const B_m = geom.width / 1000;
          const L_m = geom.length / 1000;

          const kz = ks * B_m * L_m;
          const I_xx = (1 / 12) * B_m * Math.pow(L_m, 3);
          const I_yy = (1 / 12) * L_m * Math.pow(B_m, 3);
          const krx = ks * I_xx * 1e6;
          const kry = ks * I_yy * 1e6;

          computedSprings = [0, 0, kz, krx, kry, 0];
          springRestraints = [true, true, false, false, false, true];
        }
      }
    }

    const hasColBelow = columns.some(oc =>
      !oc.isRemoved &&
      Math.abs(oc.x * 1000 - x) < 50 &&
      Math.abs(oc.y * 1000 - y) < 50 &&
      Math.abs((oc.zTop ?? ((oc.zBottom ?? 0) + oc.L)) - z) < 10
    );

    let nodeRest: [boolean, boolean, boolean, boolean, boolean, boolean] = [...restraints];
    let nodeSp: [number, number, number, number, number, number] | undefined = undefined;

    // Use support assignment or custom restraints ONLY if this node is a base (no active column segment below it)
    if (!hasColBelow) {
      // Check custom structural supports assigned from database
      const supportAssignment = supportDb && Array.isArray(supportDb.assignments)
        ? supportDb.assignments.find((a: any) => a.nodeId === searchKey)
        : null;

      if (supportAssignment) {
        const supId = supportAssignment.supportId;
        const res = supportDb.restraints?.[supId];
        if (res) {
          nodeRest = [res.ux, res.uy, res.uz, res.rx, res.ry, res.rz];
        }
        const spr = supportDb.springs?.[supId];
        if (spr && (spr.kx > 0 || spr.ky > 0 || spr.kz > 0 || spr.krx > 0 || spr.kry > 0 || spr.krz > 0)) {
          nodeSp = [spr.kx, spr.ky, spr.kz, spr.krx, spr.kry, spr.krz];
        }
      } else if (customRestraint) {
        nodeRest = [
          customRestraint.ux,
          customRestraint.uy,
          customRestraint.uz,
        ...
        if (computedSprings) {
          nodeSp = [0, 0, 0, 0, 0, 0];
          for (let k = 0; k < 6; k++) {
            if (!nodeRest[k] && computedSprings[k] > 0) {
              nodeSp[k] = computedSprings[k];
            }
          }
        }
      } else if (springRestraints && computedSprings) {
        nodeRest = springRestraints;
        nodeSp = computedSprings;
      }
    } else {
      // If there is a column below, this is an internal continuous column junction, so no external support can be applied
      nodeRest = [false, false, false, false, false, false];
      nodeSp = undefined;
    }

    const node = registry.getOrCreateNode(x, y, z, nodeRest);
    const prev = nodeRestraints.get(node.id);
    if (!prev) {
      nodeRestraints.set(node.id, [...nodeRest]);
    } else {
    ...
    if (nodeSp) {
      const prevSp = nodeSprings.get(node.id);
      if (!prevSp) {
        nodeSprings.set(node.id, [...nodeSp]);
      } else {
        const mergedSp: [number, number, number, number, number, number] = [
          prevSp[0] + nodeSp[0],
          prevSp[1] + nodeSp[1],
          prevSp[2] + nodeSp[2],
          prevSp[3] + nodeSp[3],
        ...
        nodeSprings.set(node.id, mergedSp);
      }
    }
  ...
  for (const col of columns) {
    ...
    // A column bottom is a foundation base node if there is no active column segment directly below it
    const hasColBelow = columns.some(oc =>
      !oc.isRemoved &&
      oc.id !== col.id &&
      Math.abs(oc.x - col.x) < 0.05 &&
      Math.abs(oc.y - col.y) < 0.05 &&
      Math.abs((oc.zTop ?? ((oc.zBottom ?? 0) + oc.L)) - zBot) < 10
    );
    const isBaseNode = !hasColBelow;
  ...
  const snapToColumnCenter = (xMm: number, yMm: number, zMm: number): [number, number] => {
    for (const rz of colRigidZones) {
      const atTop = Math.abs(zMm - rz.zTop) <= 100;
      const atBottom = Math.abs(zMm - rz.zBottom) <= 100;
      if (!atTop && !atBottom) continue;            // must be at same level (100 mm tol)
  ...
  const nodes3d: Node3D[] = registry.getAllNodes().map(n => {
    ...
    const springs = nodeSprings.get(n.id);
    if (springs) {
...
function runPatternEnvelope3D(
  ...
  if (model.elements.length === 0 || patternCases.length === 0) {
    return { beamEnvelope, colEnvelope, primaryBeamSplitIds, serviceColAxial, supportReactions: [] };
  }
  ...
  // Create a map from model nodeId (e.g. "N1") to coordinate-based key (e.g. "3.00_0.00_-2000")
  const nodeKeyMap = new Map<string, string>();
  for (const n of model.nodes) {
    const key = `${(n.x / 1000).toFixed(2)}_${(n.y / 1000).toFixed(2)}_${n.z.toFixed(0)}`;
    nodeKeyMap.set(n.id, key);
  }

  // Extract support reactions from multiResults
  const supportReactions: any[] = [];
  for (let caseIdx = 0; caseIdx < multiResults.length; caseIdx++) {
    const rResult = multiResults[caseIdx];
    const lc = patternCases[caseIdx];
    const loadCaseName = lc.name || lc.id;
    if (rResult.reactions) {
      for (const [nodeId, r] of rResult.reactions.entries()) {
        const clean = (val: number) => Math.abs(val) < 1e-4 ? 0 : Number(val.toFixed(4));
        const coordNodeId = nodeKeyMap.get(nodeId) || nodeId;
...
export function getFrameResults3D(
  ...
  const beamsMap = new Map(beams.map(b => [b.id, b]));
  const { beamEnvelope, primaryBeamSplitIds, supportReactions } = runPatternEnvelope3D(
    frames, beams, columns, mat, frameEndReleases, beamOnBeamConnections,
    slabs, slabProps, useFEMLoadDistribution, beamStiffnessFactor, colStiffnessFactor,
    colRigidEndOffsets, manualJointOverrides,
    supportRestraints,
  ...
  const results = frames.map((frame): FrameResult => {
  ...
  (results as any).supportReactions = supportReactions;
  return results;
}
