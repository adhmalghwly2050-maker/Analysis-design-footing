...
export default function ExportPanel({
  ...
  foundationResults, foundationMat, bentUpResults, ribbedSlabProps, colLoads3D,
}: ExportPanelProps) {
    ...
    try {
      ...
      for (let fi = 0; fi < stories.length; fi++) {
        ...
        if (format === 'pdf' || format === 'both' || format === 'print') {
          // Construction sheets (beam layout, column layout, slab plan or foundation package)
          const hasSelectedSuper = drawingTypes.beamLayout || drawingTypes.columnLayout || drawingTypes.slabPlan;
          const hasSelectedFnd = drawingTypes.foundationPlan || drawingTypes.fndSchedule || drawingTypes.fndDetails || drawingTypes.fndBbs || drawingTypes.fndTakeoff || drawingTypes.fndNotes;

          if (hasSelectedSuper || hasSelectedFnd) {
            if (format === 'print') {
              ...
              openHTMLSheetsForPrint(
                ...
                projectName, {
                  ...
                  colLoads3D,
                  columns,
                } as any, sheetSize, slabProps, mat,
                ribbedSlabProps
              );
            } else {
              if (hasSelectedSuper) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Drawing types */}
        <div>
          <p className="text-xs font-semibold mb-2 text-primary border-b pb-1">لوحات المبنى والأسقف (Superstructure):</p>
          ...
          <p className="text-xs font-semibold mb-2 text-blue-600 dark:text-blue-400 border-b pb-1">لوحات الأساسات والقواعد (Substructure - S-101 onwards):</p>
          <div className="grid grid-cols-2 gap-1">
            {([
              ['foundationPlan', 'مخطط الأساسات المنفردة (S-101)', 'لوحة مسقط توزيع الأساسات ومحاور القواعد العام'],
              ['fndSchedule', 'جدول نماذج القواعد المعتمد (S-201)', 'جدول تفاصيل الأبعاد والتسليح لنماذج الأساسات المعتمدة'],
              ['fndDetails', 'تفاصيل تسليح وقطاعات إنشائية (S-301)', 'قطاعات تفصيلية نموذجية ونقاط ارتكاز الأعمدة ورقابها بالأساس'],
              ['fndBbs', 'كشف تفريد حديد تسليح الأساسات (S-401)', 'جدول حصر وتفريد حديد التسليح التفصيلي لقواعد التأسيس'],
              ['fndTakeoff', 'بيان مساحات ومذكرة كميات المواد (S-402)', 'التقرير الفني لحصر وتكلفة الخرسانة والحديد ومواد التأسيس'],
              ['fndNotes', 'الملاحظات الإنشائية للأساسات (NOTES)', 'المواصفات العامة والتفاصيل المعيارية لتنفيذ وصب القواعد'],
            ] as [string, string, string][]).map(([key, label, desc]) => (
              <label key={key} className="flex items-center gap-2 text-[11px] cursor-pointer p-1 rounded hover:bg-muted/50 text-blue-800 dark:text-blue-300" title={desc}>
                <Checkbox
                  checked={(drawingTypes as any)[key]}
                  onCheckedChange={(v) => setDrawingTypes(p => ({ ...p, [key]: !!v }))}
                />
